# move 玻璃 marker 子函数（仅非线性玻璃）：@s = 玻璃中心 marker（tag note_glass_center）
...
# ★ 仅非线性玻璃使用（线性玻璃 marker 已由 marker_self 自算，O(N²) 消除）；由 move 调用（@s[tag=note_stained_glass,tag=!note_linear]）
# 执行后 move 外层 @s 恢复为展示实体
