...
# 宏参数：fb_hitsound（音效组号）、fb_particles（粒子组号）、fb_type（音符 type）、fb_nid（音符 id）、case_name（情况名）
...
#arg: fb_hitsound, fb_particles, fb_type, fb_nid, case_name
# 清残留（上次反馈的值；查表失败时不误播）
...
# 查音效表：feedback.sounds[组][音符type].(情况) → cur_sound → 播放
$execute if data storage rhythm_axe:feedback sounds[$(fb_hitsound)][$(fb_type)].$(case_name) run data modify storage rhythm_axe:runtime cur_sound set from storage rhythm_axe:feedback sounds[$(fb_hitsound)][$(fb_type)].$(case_name)
...
# 查粒子表：feedback.particles[组][音符type].(情况) → cur_particle → 播放
$execute if data storage rhythm_axe:feedback particles[$(fb_particles)][$(fb_type)].$(case_name) run data modify storage rhythm_axe:runtime cur_particle set from storage rhythm_axe:feedback particles[$(fb_particles)][$(fb_type)].$(case_name)
