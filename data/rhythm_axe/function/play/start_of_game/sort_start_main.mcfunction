...
# ★ 2026-09-19：5t → 10t，等 mod 的 /preloadmusic 后台解码（start 开头已发出）
#   目的：让 time==0 的 /playmusic 尽量命中缓存，避免「游戏已开始、音乐还没出声」；
#   解码慢于这个等待时 mod 仍会挂起补播（起点补偿），音乐不会丢。
schedule function rhythm_axe:play/main_loop 10t
