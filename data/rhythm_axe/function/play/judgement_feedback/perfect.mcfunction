# 判定反馈组表（M2-H）：按音符 hitsound/hit_particles 组号查表播放音效/粒子 + 执行 hit_events（情况=perfect）
...
data modify storage rhythm_axe:runtime case_name set value "perfect"
...
#当前判定计数+1
scoreboard players add perfect play_state 1
...
execute if score feedback_actionbar options matches 1 run title @a[team=player] actionbar \
    [{"text":"(","color":"red"},\
     ...
     {"text":")","color":"white","bold":true}]
execute if score feedback_chat options matches 1 run tellraw @a[team=player] [{"text":"*  PERFECT *","color":"gold"}]
