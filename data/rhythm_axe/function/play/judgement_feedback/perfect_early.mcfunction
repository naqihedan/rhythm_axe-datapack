# 判定反馈组表（M2-H）：按音符 hitsound/hit_particles 组号查表播放音效/粒子 + 执行 hit_events（情况=perfect_early）
...
data modify storage rhythm_axe:runtime case_name set value "perfect_early"
function rhythm_axe:play/feedback/feedback with storage rhythm_axe:runtime

#当前判定计数+1
scoreboard players add perfect_early play_state 1
