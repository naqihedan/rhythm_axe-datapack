# 编辑器音符击打触发执行（宏参数：nid, hitsound, hit_particles, note_type, idx, case）
# @s = 展示实体（at 其位置播击打音效/粒子）
# ★ 情况键 $(case)：自动预览恒为 perfect；真实判定 = 判定等级对应情况（bad/good_early/.../good_late/miss）
#   音效/粒子查表用同一键；hit_events 的 enabled.$(case) 也随之间变（miss 时查 miss 情况）
# ★ 音符判定时刻（playhead==time）即在判定位置 Pos，音效/粒子用 ~ ~ ~（=Pos）播放，玩家在附近可听到/看到
# hit_events 自定义指令：editor_play_events=1 时才执行（与谱面事件点同一个开关；编辑器专属反馈链，与游玩系统互不干扰）
# ★ 全部使用 storage rhythm_axe:editor.runtime（编辑器专属，不用游玩系统的 rhythm_axe:runtime）
#arg: cursor, nid, hitsound, hit_particles, note_type, idx, case
# ★ 玻璃（type=4）零反馈：任何路径到 trigger_ 都直接 return（终极防线第 4 层）
execute if score @s editor_n_type matches 4 run return fail

# 每次触发先清空上一颗音符的查表结果，避免缺失字段沿用旧反馈。
...
execute if data storage rhythm_axe:editor.runtime cur_sound run data remove storage rhythm_axe:editor.runtime cur_sound
execute if data storage rhythm_axe:editor.runtime cur_particle run data remove storage rhythm_axe:editor.runtime cur_particle
# hit_events：editor_play_events=1 → 复制该音符 hit_events 到 editor.runtime 并执行（情况 = $(case)）
# ★ 无 hit_events 数据时也存默认空列表并走执行流程（以默认值执行：无特效指令）
$execute if score editor_play_events options matches 1 run data modify storage rhythm_axe:editor.runtime case_name set value "$(case)"
$execute if score editor_play_events options matches 1 if data storage rhythm_axe:maps.editor history[$(cursor)].notes[$(idx)].hit_events run data modify storage rhythm_axe:editor.runtime hit_events.$(nid) set from storage rhythm_axe:maps.editor history[$(cursor)].notes[$(idx)].hit_events
$execute if score editor_play_events options matches 1 unless data storage rhythm_axe:maps.editor history[$(cursor)].notes[$(idx)].hit_events run data modify storage rhythm_axe:editor.runtime hit_events.$(nid) set value []
# run_hit_events 按宏参数 fb_nid 读 hit_events.$(fb_nid)：编辑器此处 fb_nid = 音符 id（nid）
$execute if score editor_play_events options matches 1 run data modify storage rhythm_axe:editor.runtime fb_nid set value $(nid)
