# 编辑器音符击打触发执行（宏参数：nid, hitsound, hit_particles, idx, case）
...
#arg: cursor, nid, hitsound, hit_particles, idx, case
# ★ 玻璃（type=4）零反馈：任何路径到 trigger_ 都直接 return（终极防线第 4 层）
execute if score @s editor_n_type matches 4 run return fail

# 播放击打音效（查表 feedback.sounds[组].$(case)）
$execute if data storage rhythm_axe:feedback sounds[$(hitsound)].$(case) run data modify storage rhythm_axe:editor.runtime cur_sound set from storage rhythm_axe:feedback sounds[$(hitsound)].$(case)
...
# 播放击打粒子（查表 feedback.particles[组].$(case)）
$execute if data storage rhythm_axe:feedback particles[$(hit_particles)].$(case) run data modify storage rhythm_axe:editor.runtime cur_particle set from storage rhythm_axe:feedback particles[$(hit_particles)].$(case)
...
# hit_events：editor_note_hitevents=1 → 复制该音符 hit_events 到 editor.runtime 并执行（情况 = $(case)）
# ★ 无 hit_events 数据时也存默认空列表并走执行流程（以默认值执行：无特效指令）
$execute if score editor_note_hitevents options matches 1 run data modify storage rhythm_axe:editor.runtime case_name set value "$(case)"
