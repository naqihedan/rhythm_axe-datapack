# 按 id 取消选中：find 其工作副本下标 → 移除 selected → 重建 selection（@s = 玩家）
# 前置：prop.nid（音符 id）
...
execute if data storage rhythm_axe:prop found_index run function rhythm_axe:editor/menu/note/selected/sel_deselect_one with storage rhythm_axe:prop
