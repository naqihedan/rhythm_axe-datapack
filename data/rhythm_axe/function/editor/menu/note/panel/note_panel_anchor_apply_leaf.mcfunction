#arg:idx
# 应用锚点变换叶子（宏，不递归）：读 selection[$(idx)] → find_by_id（顺序游标 flip_cursor；未命中兜底从 0 全扫）→ 调 apply_one 套用变换
...
execute if data storage rhythm_axe:prop found_index run function rhythm_axe:editor/menu/note/panel/note_panel_anchor_apply_one with storage rhythm_axe:prop
