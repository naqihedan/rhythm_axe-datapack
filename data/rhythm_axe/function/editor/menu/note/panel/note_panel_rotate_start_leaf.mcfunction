#arg:idx
# 旋转 start_pos 叶子（宏，不递归）：读 selection[$(idx)] → find_by_id（顺序游标 flip_cursor；未命中兜底从 0 全扫）→ 调 start_one
...
execute if data storage rhythm_axe:prop found_index run function rhythm_axe:editor/menu/note/panel/note_panel_rotate_start_one with storage rhythm_axe:prop
