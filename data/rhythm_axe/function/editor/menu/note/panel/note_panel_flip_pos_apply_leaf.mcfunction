#arg:idx
# 判定位置轴镜像应用叶子（宏函数，不递归）：读 selection[$(idx)] → find_by_id（顺序游标 flip_cursor；未命中兜底从 0 全扫）→ 调 apply_one 原位改 position
...
execute if data storage rhythm_axe:prop found_index run function rhythm_axe:editor/menu/note/panel/note_panel_flip_pos_apply_one with storage rhythm_axe:prop
