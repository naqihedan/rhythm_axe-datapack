# 音符面板相对/绝对开关：#rel_field = 1时间 / 2大小 / 3位置 / 4起始位置 / 5持续时长 / 6基础寿命
...
# ★ 取消相对（切到绝对）＝ 视为「已修改」：批量时补打 batch_set 标记 → 行首 [x] 变红，且在【确认】时写回音符
#   （即使没点 ± 也会被应用 —— 绝对模式的语义就是「把选中的都设成面板上的这个值」）
execute if score #rel_field editor matches 1 if score #rel_was editor matches 1 if data storage rhythm_axe:maps.editor editing.batch run data modify storage rhythm_axe:maps.editor editing.batch_set.time set value 1b
...
# ★ 取消相对（切到绝对）＝ 视为「已修改」：批量时补打 batch_set 标记 → 行首 [x] 变红，且在【确认】时写回音符
#   （即使没点 ± 也会被应用 —— 绝对模式的语义就是「把选中的都设成面板上的这个值」）
execute if score #rel_field editor matches 2 if score #rel_was editor matches 1 if data storage rhythm_axe:maps.editor editing.batch run data modify storage rhythm_axe:maps.editor editing.batch_set.size set value 1b
...
# ★ 取消相对（切到绝对）＝ 视为「已修改」：批量时补打 batch_set 标记 → 行首 [x] 变红，且在【确认】时写回音符
#   （即使没点 ± 也会被应用 —— 绝对模式的语义就是「把选中的都设成面板上的这个值」）
execute if score #rel_field editor matches 3 if score #rel_was editor matches 1 if data storage rhythm_axe:maps.editor editing.batch run data modify storage rhythm_axe:maps.editor editing.batch_set.position set value 1b
...
# ★ 取消相对（切到绝对）＝ 视为「已修改」：批量时补打 batch_set 标记 → 行首 [x] 变红，且在【确认】时写回音符
#   （即使没点 ± 也会被应用 —— 绝对模式的语义就是「把选中的都设成面板上的这个值」）
execute if score #rel_field editor matches 4 if score #rel_was editor matches 1 if data storage rhythm_axe:maps.editor editing.batch run data modify storage rhythm_axe:maps.editor editing.batch_set.start_pos set value 1b
...
# ★ 取消相对（切到绝对）＝ 视为「已修改」：批量时补打 batch_set 标记 → 行首 [x] 变红，且在【确认】时写回音符
#   （即使没点 ± 也会被应用 —— 绝对模式的语义就是「把选中的都设成面板上的这个值」）
execute if score #rel_field editor matches 5 if score #rel_was editor matches 1 if data storage rhythm_axe:maps.editor editing.batch run data modify storage rhythm_axe:maps.editor editing.batch_set.duration set value 1b
...
# 基础寿命
scoreboard players set #rel_was editor 0
execute if score #rel_field editor matches 6 run execute store result score #rel_was editor run data get storage rhythm_axe:maps.editor editing.rel.on.base_life
execute if score #rel_field editor matches 6 if score #rel_was editor matches 0 run data modify storage rhythm_axe:maps.editor editing.rel.on.base_life set value 1b
execute if score #rel_field editor matches 6 if score #rel_was editor matches 1 run data modify storage rhythm_axe:maps.editor editing.rel.on.base_life set value 0b
# ★ 取消相对（切到绝对）＝ 视为「已修改」：批量时补打 batch_set 标记 → 行首 [x] 变红，且在【确认】时写回音符
#   （即使没点 ± 也会被应用 —— 绝对模式的语义就是「把选中的都设成面板上的这个值」）
execute if score #rel_field editor matches 6 if score #rel_was editor matches 1 if data storage rhythm_axe:maps.editor editing.batch run data modify storage rhythm_axe:maps.editor editing.batch_set.base_life set value 1b
execute if score #rel_field editor matches 6 if score #rel_was editor matches 0 run data modify storage rhythm_axe:maps.editor editing.rel.delta.base_life set value 0
