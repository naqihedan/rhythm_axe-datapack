#arg:idx
# 判定位置轴镜像扫描叶子（宏函数，不递归）：读 selection[$(idx)] → find_by_id（顺序游标 flip_cursor；未命中兜底从 0 全扫）
#   （selection 现在按 notes 顺序递增，顺序游标本可 O(n)；兜底保证乱序也不漏）
...
execute if data storage rhythm_axe:prop found_index run function rhythm_axe:editor/menu/note/panel/note_panel_flip_pos_scan_one with storage rhythm_axe:prop
