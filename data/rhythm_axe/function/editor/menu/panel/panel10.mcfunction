# 面板 10：音符活跃列表（规范v2：1 返回；100000..103999 动态行；11301..11307 固定控件；11601/11602 翻页）。
#   动态行：值 = 100000 + 页内序×100 + 列码（复选框 0 / 编辑 3 / 复制 5 / 粘贴 6 / 删除 7；页内序 0..39）
#   固定：11301 批量编辑 / 11302 批量复制 / 11308 批量剪切 / 11303 批量粘贴 / 11306 粘贴并选中 / 11307 批量删除 / 11304 全选 / 11305 取消选中 / 11601 上一页 / 11602 下一页
#   翻转组沿用固定值 11501(翻转时间) / 11502,11503,11504,11505(X/Y/Z/S 开关) / 11506(执行翻转) / 11507,11508,11509(15°/45°/90°旋转) / 11510(锚点重置【⌖】)（面板 10/18 共用）
...
execute if score #click_value editor matches 11306 run function rhythm_axe:editor/menu/note/list/note_list_paste_select
# 批量删除（直接删除全部选中音符，不弹二次确认；一次快照可撤销；删完回活跃列表）
execute if score #click_value editor matches 11307 run function rhythm_axe:editor/note/delete/batch_delete
# 批量剪切（11308）：剪切 = 复制到剪贴板 + 删除原音符（剪贴板保留 → 可接着【批量粘贴】）
execute if score #click_value editor matches 11308 run function rhythm_axe:editor/note/cut/batch_cut
# 批量编辑（1562，进面板 11）
...
# 旋转（11507=15° / 11508=45° / 11509=90°）：写角度 cos/sin（×10000）后按 [X][Y][Z] 开关绕锚点轴旋转（中心 = 锚点实体位置，见 menu/note/anchor）
