...
# 可食用（创造模式使用不消耗）+ custom_data；右键由 use__ 分派（editor_tool_*）
...
# 可食用；custom_data 标记 editor_tool_timing；name/model 由 tool_regular_apply 按蹲下状态动态改写（全背包）
...
# 前进一刻（金锭外观；站立前进/蹲下快退；名由 tool_regular_apply 动态改写）
