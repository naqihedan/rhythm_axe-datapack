# 按音符 id 查找：普通函数驱动，规避 26.x 宏递归"幽灵重跑"（同 note_find_alive 修复）
# 前置：prop.cursor、prop.index（起始 0）、prop.note_id；命中写 prop.found_index
# 退出标志 #find_done 由叶子设置：1=命中 2=元素缺失=遍历结束 0=未命中继续
scoreboard players set #find_done editor 0
function rhythm_axe:editor/util/find_by_id_leaf with storage rhythm_axe:prop
